...
try {
  ...
  await sequelize.transaction(async (tx) => {
    ...
    if (!source?.productId || !source?.storeId || !source?.userId) {
      throw new Error("No eligible source row found for po_create_received smoke test");
    }

    const poId = randomUUID();
    const poItemId = randomUUID();
    const poNumber = `SMOKE-PO-${new Date().getFullYear()}-${Date.now().toString().slice(-4)}`;
    const nextCostBase = Number(source.costBase ?? 0) + 10;
    ...
    await sequelize.query(
      `
        ...
        values (
          ...
          'PURCHASE',
          :poId,
          'smoke purchase receive immediately',
          :userId
        )
      `,
      {
    ...
    const [checkRows] = await sequelize.query(
      `
        select
          ...
          count(*) filter (where poi.purchase_order_id = :poId and poi.qty_received = 2)::int as "itemCount"
    ...
    if (Number(checks?.movementCount ?? 0) < 1 || Number(checks?.itemCount ?? 0) < 1) {
      throw new Error("PO create received smoke validation failed");
    }
  ...
  if (error instanceof Error && error.message === "ROLLBACK_SMOKE") {
    console.info("[pg:smoke:po_create_received] ok (transaction rolled back)");
    process.exit(0);
  }

  console.error("[pg:smoke:po_create_received] failed");
