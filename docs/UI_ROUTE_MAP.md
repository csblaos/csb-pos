...
| Page | Main Component | API Calls |
|---|---|---|
| `/stock?tab=inventory` | `components/app/stock-inventory-view.tsx` | `GET /api/stock/products?page&pageSize` (รีเฟรช/โหลดเพิ่มรายการสินค้าในแท็บดูสต็อก), `GET /api/products/search?q&includeStock=true` (resolve ผลสแกนบาร์โค้ดให้แม่นขึ้นด้วยการหา exact barcode ก่อน fallback); sync state ลง URL เฉพาะตอนแท็บ active ผ่าน `inventoryQ/inventoryFilter/inventorySort`; scanner UI/logic ใช้คอมโพเนนต์กลางเดียวกับ `/products` |
