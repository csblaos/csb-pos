...
| Page | Main Component | API Calls |
|---|---|---|
| `/stock?tab=history` | `components/app/stock-movement-history.tsx` | `GET /api/stock/movements?view=history&page&pageSize&type&q&productId&dateFrom&dateTo` (history server-side pagination/filter; list render แบบ windowed virtualization; UI sync filter/page ลง URL ด้วย `historyType/historyQ/historyDateFrom/historyDateTo/historyPage`, มี chip filter type ครบถึง `RESERVE/RELEASE`, และมี in-memory cache ต่อ filter key เพื่อสลับมุมมองเดิมได้เร็วขึ้น) |
| `/stock?tab=recording` | `components/app/stock-recording-form.tsx` | `POST /api/stock/movements` (ส่ง `Idempotency-Key` จาก client; บันทึก qty/unit/movementType; ถ้าส่ง field cost/rate จะโดน reject 400), `GET /api/stock/movements` (รีเฟรชสินค้าในแท็บ); UI มี guardrail/CTA ไปแท็บ PO โดยส่วนรายละเอียดยาวเป็นแบบพับ/ขยาย (default ปิด), มี mobile product picker (`ดูสินค้าทั้งหมด`) + sticky action `บันทึกสต็อก`, และ sync filter ลง URL (`recordingType`, `recordingProductId`) |
