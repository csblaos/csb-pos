...
- Orders:
  ...
  - UX `/orders/new`:
    ...
    - flow `สั่งออนไลน์/จัดส่ง` ปรับ UX กรอกลูกค้า/ที่อยู่:
      ...
      - เพิ่ม section `ข้อมูลขนส่ง` สำหรับ online flow: เริ่มต้นค่า `ผู้ให้บริการขนส่ง` เป็นว่างเสมอ (ไม่ auto select), ผู้ใช้ต้องเลือกเองก่อนสร้างออเดอร์; UI เป็นปุ่ม grid (`Houngaloun`, `Anousith`, `Mixay`, `อื่นๆ`) และถ้าเลือก `อื่นๆ` กรอกชื่อผู้ให้บริการเพิ่มได้ (ไม่มีช่องสาขารับฝากในฟอร์มแล้ว)
    ...
    - อัปเดต flow COD ในหน้า detail:
      ...
      - `confirm_paid` สำหรับ COD เปลี่ยนเป็นปุ่ม `ยืนยันรับเงินปลายทาง (COD)` และใช้ได้หลังสถานะ `SHIPPED` เท่านั้น (อัปเดต `paymentStatus` เป็น `COD_SETTLED` โดยไม่เปลี่ยน `status`)
    ...
    - หน้า `/reports` เพิ่มการ์ด `สรุป COD` (ค้างเก็บ, ปิดยอดวันนี้, ตีกลับวันนี้, COD สุทธิสะสม) และตารางย่อย `แยกตามขนส่ง` โดย metric รายวันของการตีกลับอ้างอิง `cod_returned_at`
    - การยืนยันชำระ (`confirm_paid`) ของออเดอร์ทั่วไปยังรองรับสถานะ `PENDING_PAYMENT` และ `READY_FOR_PICKUP`; การยกเลิกจาก `READY_FOR_PICKUP` จะปล่อยจองสต็อกกลับ
    - flow เป็น `เลือกสินค้าในหน้า POS` -> `เปิดตะกร้า/กดชำระเงิน` -> `Checkout sheet` เพื่อกรอกลูกค้า/ชำระเงิน/ที่อยู่ แล้วค่อยบันทึก
    - หลังสร้างออเดอร์สำเร็จ: `Walk-in` และ `รับที่ร้านภายหลัง` จะแสดง success action sheet (พิมพ์ใบเสร็จ/ใบรับสินค้า, ดูรายละเอียด, หรือเริ่มออเดอร์ใหม่); ส่วน `ออนไลน์/จัดส่ง` จะพาไปหน้า detail ทันที
    - success action sheet ของ `Walk-in/Pickup` แสดงตัวอย่างบิลใน modal และปุ่มพิมพ์ใช้ hybrid print:
      - Desktop: พิมพ์ผ่าน hidden iframe (ไม่เปิดแท็บใหม่) และออกเป็น receipt-only
      - Mobile/Tablet: ไปหน้า `/orders/[orderId]/print/receipt?autoprint=1` ในแท็บเดิม, auto print และมีปุ่ม `กลับ POS`
